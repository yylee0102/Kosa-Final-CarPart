...
import { Calendar, User, FileText, Clock, CheckCircle, XCircle, Wrench } from 'lucide-react';
        ...
        <div className="space-y-6 pt-4">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-4 text-sm bg-muted p-4 rounded-lg">
              <div>
                <span className="font-medium text-muted-foreground">고객명:</span>{' '}
                <span className="font-medium">{estimate.customerName}</span>
              </div>
              <div>
                <span className="font-medium text-muted-foreground">차량:</span>{' '}
                <span className="font-medium">{estimate.carModel} ({estimate.carYear}년)</span>
              </div>
            </div>
          </div>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div className="p-3 bg-muted rounded-lg">
                  ...
                  <p className="font-medium mt-1">{estimate.workDuration || '정보 없음'}</p>
                </div>
                <div className="p-3 bg-muted rounded-lg">
                  <span className="text-muted-foreground">견적 유효기간</span>
                  <p className="font-medium mt-1">
                    {estimate.validUntil ? new Date(estimate.validUntil).toLocaleDateString() : '정보 없음'}
                  </p>
                </div>
              </div>
              ...
              <div className="text-sm text-muted-foreground flex items-center gap-2">
                ...
                견적서 작성일: {new Date(estimate.createdAt).toLocaleDateString()}
              </div>
            </div>
          </div>
