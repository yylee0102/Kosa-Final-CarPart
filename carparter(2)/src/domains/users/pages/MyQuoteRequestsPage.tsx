// 파일: MyQuoteRequestsPage.tsx (최종 수정본)
...
// 1. user.api.ts에서 "공식" 타입들을 직접 import 합니다.
import UserApiService, { QuoteRequestResDTO, Estimate } from '@/services/user.api';
...
export const MyQuoteRequestsPage = () => {
  ...
  // 3. useState에서 직접 import한 QuoteRequestResDTO 타입을 사용합니다.
  const [myRequest, setMyRequest] = useState<QuoteRequestResDTO | null>(null);
  ...
  const handleChatRequest = async (centerId: string, centerName: string, estimateId: number) => {
    try {
        const room = await ChatApiService.createOrGetChatRoom({centerId, estimateId});
        navigate('/chat', {
            state: { roomId: room.roomId, centerId: centerId, centerName: centerName }
        });
  ...
  return (
    <PageContainer>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <Card>
                  ...
                  <CardContent>
                      ...
                      {myRequest ? (
                      // estimates 배열 안에 ACCEPTED 또는 REJECTED 상태인 것이 하나라도 있는지 확인
                      myRequest.estimates.some(e => e.status === 'ACCEPTED' || e.status === 'REJECTED')
                    ? '견적 진행중'
                    : '견적 대기중'
                     ) : '-'}
                  </div>
            ...
            <>
              <Card>
                ...
                <CardContent className="space-y-4">
                  {/* 4. 데이터 접근 방식을 "최신" 타입에 맞게 수정합니다. */}
                  <div className="flex items-center gap-4 text-muted-foreground">
                    <div className="flex items-center gap-2">
                      <Car className="h-4 w-4" />
                      {/* 이제 myRequest.userCar 객체를 사용합니다. */}
                      <span>{myRequest.userCar?.carModel ?? '차량 정보 없음'} ({myRequest.userCar?.modelYear ?? 'N/A'}년)</span>
                    </div>
                  ...
                  {/* 5. 이미지 렌더링 방식을 단순 문자열 배열에 맞게 수정합니다. */}
                  {myRequest.imageUrls && myRequest.imageUrls.length > 0 && (
                    <div className="flex gap-2 pt-4">
                      {myRequest.imageUrls.map((imageUrl, index) => (
                        <img key={index} src={imageUrl} alt={`차량사진 ${index + 1}`} className="w-24 h-24 rounded object-cover" />
                      ))}
                    </div>
                  )}
                </CardContent>
      ...
      <EstimateDetailModal
        ...
        onConfirm={() => {
            console.log(`견적 확정!`);
            loadMyRequest(); // 확정 후 목록 새로고침
            setIsDetailModalOpen(false); // 2. 모달을 닫습니다. (이 줄 추가)
        }}
